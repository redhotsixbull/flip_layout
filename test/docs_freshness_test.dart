...
// mistakes that actually happened across this workspace: a README that restated
// a version and went stale, a CHANGELOG that lagged `pubspec.yaml`, and public
// types shipped in a release with no entry in the API reference.
//
...
/// surface. Add here only with a reason.
const _undocumentedOnPurpose = {
  'SharedElementController', // machinery behind MotionSharedScope; reachable
  // only through a private `_maybeOf`, exported because the whole library file
  // is. Documenting it would advertise an API users cannot obtain a handle to.
};
...
void main() {
  group('docs stay current', () {
    test('README carries no version literals', () {
      ...
      // README should never restate any of them.
    ...
    test('CHANGELOG top entry matches pubspec version', () {
      ...
      final newest = RegExp(r'^##\s+(\S+)', multiLine: true)
    ...
    test('every public type has an entry in doc/API.md', () {
      ...
      final declaration = RegExp(
          r'^(?:abstract\s+)?(?:class|enum)\s+([A-Z]\w*)',
          multiLine: true);
